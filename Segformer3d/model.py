...
from .dataloader import Brats2017InferSingleSample, ConvertBackToOriginalClasses
...
from .infer import display_image_channels, display_mask_channels_as_rgb, overlay_masks_on_image
...
def segformer3d_get_predict(segformer3D, case_name):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

    infer = Brats2017InferSingleSample(root_dir="dataset", case_name=case_name)
    transformed_data = infer()

    input = transformed_data['image'].unsqueeze(0).to(device)
    image = input[0].numpy()
    label = None
    gt = None
    ...
    output_trans = post_trans(output[0])
   
    pred = output_trans.detach().numpy()
...
if __name__ == '__main__':
    ...
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    segformer3D = build_architecture(model_config).to(device)

    segformer3D.load_state_dict(
        torch.load('Segformer3d/pretrain_model/segformer3d.pth', map_location=device, weights_only=True))

    image, gt, pred, label_back, pred_back = segformer3d_get_predict(segformer3D, 'Brats17_2013_10_1')
    n_slice = 55
    # display_image_channels(image, n_slice)
    # display_mask_channels_as_rgb(gt, n_slice)
    # display_mask_channels_as_rgb(pred, n_slice)
    #
    # print(image.shape)
    # print(gt.shape)
    # print(pred.shape)
    ...
    #
    # plt.subplot(121)
    # plt.imshow(label_back[:, :, n_slice])
    # plt.axis('off')
    # plt.subplot(122)
    # plt.imshow(pred_back[:, :, n_slice])
    # plt.axis('off')
    # plt.show()

    print(np.unique(pred_back[:, :, n_slice]))
